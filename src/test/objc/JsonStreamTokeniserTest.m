...
- (void)testBasics {
    ...
    STAssertEquals([tokeniser getToken:&bytes length:&length], sbjson_token_array_open, tokeniser.error);

    STAssertEquals([tokeniser getToken:&bytes length:&length], sbjson_token_bool, tokeniser.error);
    STAssertTrue(!strncmp(bytes, "true", 4), nil);

    STAssertEquals([tokeniser getToken:&bytes length:&length], sbjson_token_value_sep, tokeniser.error);
    ...
    STAssertEquals([tokeniser getToken:&bytes length:&length], sbjson_token_null, tokeniser.error);
    STAssertTrue(!strncmp(bytes, "null", 4), nil);

    STAssertEquals([tokeniser getToken:&bytes length:&length], sbjson_token_array_close, tokeniser.error);

    STAssertEquals([tokeniser getToken:&bytes length:&length], sbjson_token_eof, tokeniser.error);
}

- (void)testNumber {
    [tokeniser appendData:data(@"123 45.6 7.8e9 ")];

    STAssertEquals([tokeniser getToken:&bytes length:&length], sbjson_token_integer, tokeniser.error);
    STAssertEqualObjects(str(bytes, length), @"123", nil);

    STAssertEquals([tokeniser getToken:&bytes length:&length], sbjson_token_real, tokeniser.error);
...
- (void)testString {
    ...
    STAssertEquals([tokeniser getToken:&bytes length:&length], sbjson_token_string, tokeniser.error);
...
- (void)testEncoded {
    ...
    STAssertEquals([tokeniser getToken:&bytes length:&length], sbjson_token_encoded, tokeniser.error);
